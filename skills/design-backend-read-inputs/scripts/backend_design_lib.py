...
STACK_KEYWORDS = {
    "language": ["python", "java", "kotlin", "go", "node", "typescript"],
    "framework": ["spring", "fastapi", "flask", "django", "express", "gin"],
    "storage": ["mysql", "postgres", "postgresql", "redis", "mongodb", "dynamodb"],
    "cache": ["redis", "memcached", "cache"],
    "mq": ["kafka", "rabbitmq", "sqs", "pubsub", "rocketmq", "topic", "queue"],
}
...
def feature_identity(final_prd: dict[str, Any]) -> tuple[str, str]:
    feature_id = (
        ...
        or final_prd.get("id")
        or slugify(str(final_prd.get("title") or final_prd.get("name") or "backend_feature"))
    )
...
def merge_repo_context(
    ...
    service_inventory: dict[str, Any] | None,
    architecture_constraints: dict[str, Any] | None,
    existing_api_specs_dir: str | None,
) -> dict[str, Any]:
    merged = dict(repo_context)
    inventory = service_inventory or {}
    constraints = architecture_constraints or {}
    merged["services"] = as_list(merged.get("services")) or as_list(inventory.get("services"))
    merged["modules"] = as_list(merged.get("modules")) or as_list(inventory.get("modules"))
    merged["owned_domains"] = as_list(merged.get("owned_domains")) or as_list(inventory.get("owned_domains"))
    merged["layering_rules"] = as_list(merged.get("layering_rules")) or as_list(constraints.get("layering_rules"))
    merged["dependency_constraints"] = as_list(merged.get("dependency_constraints")) or as_list(constraints.get("dependency_constraints"))
    merged["integration_constraints"] = as_list(merged.get("integration_constraints")) or as_list(constraints.get("integration_constraints"))
    if existing_api_specs_dir:
        specs_dir = Path(existing_api_specs_dir)
        if specs_dir.exists():
            merged["existing_apis"] = merged.get("existing_apis") or [
                {
                    "id": slugify(spec.stem, "api"),
                    "method": "UNKNOWN",
                    "path": str(spec.name),
                    "summary": "Imported from existing_api_specs/",
                }
...
def needs_api_specs(final_prd: dict[str, Any]) -> bool:
    text = " ".join(requirement_lines(final_prd)).lower()
    keywords = ["api", "endpoint", "interface", "event", "webhook", "job"]
...
def scan_contract_ids(path: str | Path) -> dict[str, list[str]]:
    ids = {"api": [], "event": [], "job": []}
...
def top_level_yaml_keys(path: str | Path) -> list[str]:
    keys: list[str] = []
