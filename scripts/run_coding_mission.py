...
def select_ready_tasks(task_graph: dict[str, Any], max_tasks: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for task in tasks:
        ...
        # Note: changed_files is a coding OUTPUT (populated after implementation),
        # not a readiness prerequisite. Do NOT gate task selection on it.
        if missing_deps or blocked or not assets_ready or not checkpoint_match or not has_design_refs:
            reason = []
            if missing_deps:
...
def build_task_execution_report(selected_tasks: list[dict[str, Any]], hook_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for task in selected_tasks:
        ...
        all_planned = bool(task_hooks) and all(item.get("status") == "planned" for item in task_hooks)
        status = "failed" if failed else ("planned" if all_planned else "completed")
...
def build_implementation_evidence(inputs: dict[str, Any], selected_tasks: list[dict[str, Any]], hook_results: list[dict[str, Any]], execute_evidence: bool) -> dict[str, Any]:
...
def build_drift_issues(selected_batch: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for task in selected_batch.get("selected_tasks", []):
        ...
        if not task.get("design_artifact_refs"):
            # high: design traceability is mandatory — every task must link back to a design artifact
        ...
        if not task.get("changed_files"):
            # medium: changed_files is populated by execution; pre-declaration is encouraged but not blocking
            issues.append({"id": f"drift-changed-files-{task_id}", "summary": f"task {task_id} has no declared changed_files — populate after implementation", "severity": "medium"})
    return issues
...
def run() -> int:
    ...
    task_report = build_task_execution_report(selected_tasks, hooks)
    changed = build_changed_files(selected_tasks)
    evidence = build_implementation_evidence(inputs, selected_tasks, hooks, args.execute_evidence)
