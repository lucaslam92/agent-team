...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Execute backend tasks from selected_task_batch.json")
    parser.add_argument("--selected-task-batch", required=True)
    parser.add_argument("--output", default="artifacts/coding/backend_task_execution.json")
...
def run() -> int:
    ...
    selected = [task for task in batch.get("selected_tasks", []) if str(task.get("endpoint", "")).startswith("backend")]
